...
export default function SkillTreePage() {
  ...
  const handleConfirmInput = () => {
    ...
    if (pendingAction === "root" && currentTree) {
      ...
      const newNode: SkillNode = { id: `node-${Date.now()}`, label: inputValue, x: selectedNode.x + 100, y: selectedNode.y + 100, parentId: selectedNode.id };
      setTrees(prevTrees =>
        prevTrees.map(tree =>
      ...
      setTrees(prevTrees =>
        prevTrees.map(tree =>
          tree.id === currentTree.id
            ? { ...tree, nodes: tree.nodes.map(n => (n.id === selectedNode.id ? { ...n, label: inputValue } : n)) }
            : tree
        )
      );
    }
      ...
      <>
        <Svg height="100%" width="100%" style={{ position: "absolute" }}>
          {currentTree.nodes.map(node => {
            ...
            return (
              <Line key={`line-${parent.id}-${node.id}`} x1={parent.x} y1={parent.y} x2={node.x} y2={node.y} stroke={theme.colors.primary} strokeWidth="2" />
            );
          })}
        </Svg>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  height: 60,
                  borderRadius: 30,
                  backgroundColor: isSelected ? theme.colors.primary : isParentUnlocked ? theme.colors.secondaryContainer : "#555",
                  borderWidth: 2,
    ...
    <SafeAreaView style={{ flex: 1, flexDirection: "row", backgroundColor: theme.colors.background }}>
      <View style={{ width: 220, padding: 10, backgroundColor: theme.colors.surfaceVariant }}>
        ...
        <ScrollView style={{ marginBottom: 10 }}>
          {trees.map(tree => (
            <Pressable
              ...
              style={{ padding: 8, marginBottom: 6, backgroundColor: tree.id === selectedTreeId ? theme.colors.primary : "transparent", borderRadius: 6 }}
            >
              <Text style={{ color: tree.id === selectedTreeId ? theme.colors.onPrimary : theme.colors.onSurfaceVariant, fontWeight: "bold" }}>{tree.name}</Text>
            </Pressable>
          ))}
        </ScrollView>
        ...
        <TextInput
          ...
          style={{ borderWidth: 1, borderColor: theme.colors.primary, marginBottom: 8, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4, color: theme.colors.onSurface }}
        ...
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.3)", justifyContent: "center", alignItems: "center" }}>
          <View style={{ width: 300, backgroundColor: theme.colors.surface, borderRadius: 8, padding: 20 }}>
            ...
            <TextInput
              ...
              style={{ borderWidth: 1, borderColor: theme.colors.primary, marginBottom: 10, padding: 8, borderRadius: 6, color: theme.colors.onSurface }}
              placeholder="Enter name..."
              placeholderTextColor={theme.colors.onSurfaceVariant}
            />
            <Button mode="contained" onPress={handleConfirmInput}><Text style={{ color: theme.colors.onPrimary }}>Confirm</Text></Button>
            <Button onPress={() => setShowInputModal(false)} style={{ marginTop: 10 }}><Text style={{ color: theme.colors.primary }}>Cancel</Text></Button>
          </View>
        </View>
      </Modal>
    </SafeAreaView>
